...
func (f *funcMap) Map() template.FuncMap {
	return template.FuncMap{
		...
		// --- Custom functions for entry preview ---
...
// firstImageURL extracts the first <img src="..."> URL from HTML content.
// It uses Go's HTML tokenizer for robust parsing across all image formats
// and attribute orderings. Returns an empty string if no image is found.
...
// truncateHTML strips HTML tags and truncates the plain text to maxChars characters.
// It uses the existing sanitizer.StripTags and sanitizer.TruncateHTML functions.
func truncateHTML(content string, maxChars int) string {
	if maxChars <= 0 {
		maxChars = 200
	}
	return sanitizer.TruncateHTML(content, maxChars)
}
